...
/** Postprocessing effect that restores the original terminal contour pass. */
export class TerminalContourEffect extends Effect {
  ...
  /** Resizes the screen-space sampling grid used by the effect. */
  public setSize(width: number, height: number): void {
    this.resolutionUniform.value.set(width, height);
  }

  /** Applies current UI settings and transition fade amount to the effect. */
  public updateSettings(settings: CymaticSettings, amount = 1): void {
